    ...
    <div className="space-y-6">
      ...
      {filteredJournals.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 text-slate-500 space-y-2">
          ...
          <p className="text-sm font-bold text-slate-700">Tidak ada jurnal harian ditemukan</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredJournals.map(j => (
            <div
              key={j.id}
              className="bg-white rounded-2xl border border-slate-200/80 hover:border-indigo-300 p-5 shadow-xs hover:shadow-md transition space-y-4 flex flex-col justify-between"
            >
              <div className="space-y-3">
                ...
                {/* Subject & Teacher */}
                <div>
                  <h3 className="text-sm font-bold text-slate-900">{j.subject}</h3>
                  <p className="text-xs text-slate-500 font-medium">{j.teacherName}</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">{j.timeSlot}</p>
                </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">ID: {viewingJournal.id}</p>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs">
                <div>
                  ...
                  <span className="font-bold text-slate-900">{viewingJournal.date}</span>
                </div>
                <div>
                ...
                <div>
                  <span className="text-slate-400 font-semibold block">Guru</span>
                  <span className="font-bold text-slate-900">{viewingJournal.teacherName}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-semibold block">Mata Pelajaran</span>
                  <span className="font-bold text-slate-900">{viewingJournal.subject}</span>
                </div>
              </div>
              ...
              {viewingJournal.tpDescriptions.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold text-slate-900 mb-1">Tujuan Pembelajaran (TP)</h4>
              ...
              {/* Material Summary */}
              <div>
                <h4 className="text-xs font-bold text-slate-900 mb-1">Uraian Kegiatan / Ringkasan Materi</h4>
              ...
              {viewingJournal.incidents.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold text-slate-900 mb-2">Catatan Kejadian Siswa</h4>
                  <div className="space-y-1.5">
                    {viewingJournal.incidents.map(inc => (
                      <div key={inc.id} className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 text-xs">
                        <span className="font-bold text-slate-900">{inc.studentName}</span>: {inc.note}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {viewingJournal.photoUrl && (
                <div>
                  <h4 className="text-xs font-bold text-slate-900 mb-2">Foto Laporan Visual (Drive)</h4>
                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200 text-center space-y-2">
                    <img 
                      src={viewingJournal.photoUrl} 
                      alt="Laporan Visual" 
                      className="max-h-60 mx-auto rounded-xl object-cover border border-slate-300" 
                    />
